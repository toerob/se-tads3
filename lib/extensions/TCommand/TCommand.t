   ...
   proper(str)
   {
     foreach(local name in nilToList(properNames))
    {
      ...
      str = str.findReplace(name1, name2, ReplaceAll);      
        ...
        str = rexReplace('%<' + getActor.theName + '<`|squote>s%>', str, 'din', 
                              ReplaceAll);  
        str = rexReplace('%<' + getActor.theName + '%>', str, 'dig själv', 
                         ReplaceAll);
...
function youToMe(str)
{
  ...
  {       
   str = str.findReplace(' du ', ' jag ', ReplaceAll);
    if(str.endsWith(' du'))
      str = str.findReplace(' du', ' jag', ReplaceOnce,
        str.length-5);
  }
  return str.toLower();      
}
...
modify ActorState
  obeyCommand(issuingActor, action)
    ...
    explainBlockTopicCommand()
    {
        "Det f{a|i}anns ingen större mening med att be <<getActor.itObj>> göra det.";
    }
;
...
class SuggestedCommandTopic : SuggestedTopic
   ...
   fullName = ('be {den targetActor/honom} att ' + name)
;

suggestionCommandGroup : SuggestionListGroup
     groupPrefix = "be {den targetActor/honom} att "
;
