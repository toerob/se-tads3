...
modify BasicOpenable
    ...
    /* open/closed names */
    nameOpenClosed = ((isOpen ? 'öppna ' : 'stängda ') + name)
    theNameOpenClosed = ((isOpen ? '<<isNeuter?'det':'den'>> öppna ' : '<<isNeuter?'det':'den'>> stängda ') + name)
    pluralNameOpenClosed = ((isOpen ? 'öppna ' : 'stängda ') + name)
    aNameOpenClosed()
    {
        if (isPlural || isMassNoun)
            return (isOpen ? 'öppna ' : 'stängda ') + name;
        else {
            local art = isNeuter?'det':'den';
            return (isOpen ? '<<art>> öppen ' : '<<art>> stängd ') + name;
        }
    }
;
