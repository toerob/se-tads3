 ...
 *   
 *   Converted to adv3 by Tomas Öberg
 *
 */
...
valToSym(val) {
    local str;
    switch(dataType(val)) {
        case TypeSString:        return val;
        case TypeInt:            return toString(val);
        case TypeObject:
            str = symTab.ctab[val]; 
            if(str == '???' && val.propDefined(&name)) {
                str = val.name;
            }
            return str;
...
modify TadsObject
    objToString() {
        ...
        str  += '(' + getSuperclassList + ')'; // Append this object's superclass list in parentheses
        return str;
    }
;
...
DefineLiteralAction(DoTest)
     ...
     */
    execAction() {
        local target = gLiteral.toLower(); //getEnteredText(); //cmd.dobj.name.toLower();
...
class Test: object
    ...
    getHolding() {
        ...
        if(reportHolding && testHolding.length > 0) {
            local str = testHolding.subset({x:x.theName()}).join(', ');
            "Jag håller nu i: <<str>>";  
        }
